...
math.randomseed(math.floor((os.time() / os.clock()) * (os.time() % os.clock())))
math.random()
...
local function getUUID()
	...
	--
	local time_low_a = math.random(0, 65535)
	local time_low_b = math.random(0, 65535)
	--
	local time_mid = math.random(0, 65535)
	--
	local time_hi = math.random(0, 4095 )
	time_hi = padbits( num2bs(time_hi), 12 )
	local time_hi_and_version = bs2num( "0100" .. time_hi )
	--
	local clock_seq_hi_res = math.random(0,63)
